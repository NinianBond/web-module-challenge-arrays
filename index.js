...
function removeFlavorByName(flavors, idx){
    const index = flavors.indexOf(idx);
...
function filterByWord(flavors, promoFlavor){
    const promoFlavors = [];
    for (let i = 0; i < flavors.length; i++) {
      if (flavors[i].includes(promoFlavor)) {
        promoFlavors.push(flavors[i]);
      }
    }
    return promoFlavors;
}
...
function getAverageWordLength(flavors){
   let sum = 0;
   for (let i = 0; i < flavors.length; i++) {
    sum += flavors[i].length;
   }
   return sum / flavors.length;
}
...
function getRandomFlavors(origFlav, newFlav, seasFlav, regFlav){
  const select31 = [];
  const flavArr = [origFlav, newFlav, seasFlav, regFlav];
  for (let i = 0; i < 31; i++) {
    select31.push(randomness(randomness(flavArr)));
  }
  return select31;
